...
const defaultRules = [
  { rule_key: "repeat_click_suspicious", rule_name: "반복 클릭 의심", description: "같은 광고주와 IP 기준 10분 내 3회 이상 클릭 시 의심으로 판정합니다.", action: "suspicious", threshold: 3, risk_delta: 0, is_enabled: true },
  { rule_key: "repeat_click_block", rule_name: "반복 클릭 차단", description: "같은 광고주와 IP 기준 10분 내 5회 이상 클릭 시 차단으로 판정합니다.", action: "blocked", threshold: 5, risk_delta: 0, is_enabled: true },
  { rule_key: "short_stay", rule_name: "짧은 체류", description: "체류시간이 3초 이하이면 위험도를 가중합니다.", action: "monitor", threshold: 3, risk_delta: 12, is_enabled: true },
  { rule_key: "no_page_move", rule_name: "무이동 세션", description: "페이지 이동이 0회이면 위험도를 가중합니다.", action: "monitor", threshold: 0, risk_delta: 10, is_enabled: true },
  { rule_key: "partner_media_watch", rule_name: "제휴 매체 관찰", description: "제휴 매체 유입을 모니터링 reason에 추가합니다.", action: "monitor", threshold: null, risk_delta: 0, is_enabled: false }
];
...
export async function GET(request) {
  ...
  const { data, error } = await supabase
    .from("pm_block_rules")
    .select("id,advertiser_id,rule_key,rule_name,description,action,threshold,risk_delta,is_enabled,created_by,created_at,updated_at,advertiser:pm_advertisers(name)")
