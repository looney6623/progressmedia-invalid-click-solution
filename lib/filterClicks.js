import { trafficSourceKey, trafficSourceLabel, rawTrafficDetail } from "@/lib/trafficSource";
...
export function filterClicks(logs, filters) {
  ...
  return logs.filter((log) => {
    ...
    if (filters.media !== "전체" && filters.media !== "all" && trafficSourceKey(log) !== filters.media) return false;
    ...
    return [log.ip, log.advertiser, log.media, trafficSourceLabel(log), rawTrafficDetail(log), log.reason].some((value) => String(value).toLowerCase().includes(query));
  });
}
