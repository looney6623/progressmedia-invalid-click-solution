...
async function redirectToAdvertiserSiteUrl(request, advertiserKey) {
  if (!advertiserKey) {
    logBlock("[NAVER_TRACKING_VALIDATION]", {
      reason: "missing_advertiser_key",
      redirect_target: "",
      status: 200
    });
    return readyText();
  ...
  try {
    ...
    if (!advertiser || !siteUrl) {
      logBlock("[NAVER_TRACKING_VALIDATION]", {
        reason: advertiser ? "missing_site_url" : "advertiser_not_found",
        redirect_target: "",
        status: 200
      });
      return readyText();
    ...
    logBlock("[NAVER_TRACKING_VALIDATION]", {
      reason: "validation_final_url_missing_or_placeholder",
      redirect_target: siteUrl.href,
      status: 302
    });
    return NextResponse.redirect(siteUrl, { status: 302 });
  } catch (error) {
    logBlock("[NAVER_TRACKING_VALIDATION]", {
      reason: error?.message || "advertiser_lookup_failed",
      redirect_target: "",
      status: 200
    });
    return readyText();
...
export async function handleNaverTrackingRequest(request, { channel: rawChannel } = {}) {
  ...
  const nFinalUrl = readParam(params, "n_final_url") || readParam(params, "final_url");
  const shortUrl = readParam(params, "url");
  const referer = request.headers.get("referer") || request.headers.get("referrer") || "";

  logBlock("[NAVER_TRACKING_REQUEST]", {
    time: new Date().toISOString(),
    path: url.pathname,
    method: request.method,
    channel: channel || rawChannel || readParam(params, "c") || "",
    query: url.search || "",
    user_agent: request.headers.get("user-agent") || "",
    referer,
    pm_adv: readParam(params, "pm_adv"),
    aid: readParam(params, "aid"),
    n_final_url: nFinalUrl,
    url: shortUrl
  });

  if (!channel) {
    logBlock("[NAVER_TRACKING_ERROR]", {
  ...
  if (!advertiserKey) {
    ...
    return json({ ok: false, error: "aid or pm_adv is required" }, 400);
  }
  ...
  if (!finalUrl) {
    ...
    return json({ ok: false, error: "invalid final url" }, 400);
  }
  ...
  if (!sameAdvertiserDomain(finalUrl.href, advertiser.site_url)) {
    logBlock("[NAVER_TRACKING_ERROR]", {
      reason: "final_url_domain_not_allowed",
      status: 403
    });
  ...
  if (!ipHash) {
    logBlock("[NAVER_TRACKING_ERROR]", {
      reason: "ip_hash_salt_required",
      status: 500
    });
    return json({ ok: false, error: "IP_HASH_SALT is required" }, 500);
  }
  ...
  if (insertError) {
    logBlock("[NAVER_TRACKING_ERROR]", {
      reason: insertError.message || "click_log_insert_failed",
      status: 500
    });
