...
const fallbackRules = [
  { rule_key: "repeat_click_suspicious", rule_name: "반복 클릭 의심", action: "suspicious", threshold: 3, risk_delta: 0, is_enabled: true },
  { rule_key: "repeat_click_block", rule_name: "반복 클릭 차단", action: "blocked", threshold: 5, risk_delta: 0, is_enabled: true },
  { rule_key: "short_stay", rule_name: "짧은 체류", action: "monitor", threshold: 3, risk_delta: 12, is_enabled: true },
  { rule_key: "no_page_move", rule_name: "무이동 세션", action: "monitor", threshold: 0, risk_delta: 10, is_enabled: true },
  { rule_key: "partner_media_watch", rule_name: "제휴 매체 관찰", action: "monitor", threshold: null, risk_delta: 0, is_enabled: false }
];
...
async function fetchRules(supabase, advertiserId) {
  try {
    const { data, error } = await supabase
      .from("pm_block_rules")
      .select("rule_key,rule_name,action,threshold,risk_delta,is_enabled")
...
export async function detectInvalidClick({
  ...
  try {
    ...
    if (data) {
      ...
      return {
        ...
        risk_score: 100,
        riskScore: 100,
        reason: data.reason ? `수동 차단 IP: ${data.reason}` : "수동 차단 IP",
        recent_count: recentCount,
        recentCount,
        matched_block: matchedBlock,
        matchedBlock,
        applied_rules: [{ rule_key: "manual_block", action: "blocked" }],
        appliedRules: [{ rule_key: "manual_block", action: "blocked" }]
      };
    }
  } catch (error) {
  ...
  const blockRule = getRule(rules, "repeat_click_block");
  const suspiciousRule = getRule(rules, "repeat_click_suspicious");
  const blockThreshold = blockRule?.threshold ?? 5;
  const suspiciousThreshold = suspiciousRule?.threshold ?? 3;
  let clickStatus = "normal";

  if (blockRule && recentCount >= blockThreshold) {
    clickStatus = "blocked";
    riskScore = Math.max(riskScore, 90);
    reasons.push(`10분 내 ${blockThreshold}회 이상 반복 클릭`);
    appliedRules.push({ rule_key: blockRule.rule_key, action: "blocked", threshold: blockThreshold });
  } else if (suspiciousRule && recentCount >= suspiciousThreshold) {
    clickStatus = "suspicious";
    riskScore = Math.max(riskScore, 64);
    reasons.push(`10분 내 ${suspiciousThreshold}회 이상 반복 클릭`);
    appliedRules.push({ rule_key: suspiciousRule.rule_key, action: "suspicious", threshold: suspiciousThreshold });
  }

  const shortStayRule = getRule(rules, "short_stay");
  ...
  const noMoveRule = getRule(rules, "no_page_move");
  ...
  const partnerRule = getRule(rules, "partner_media_watch");
